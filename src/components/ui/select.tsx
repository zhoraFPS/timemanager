...
// Label registry: each SelectItem registers its children under its value so
// SelectValue can render the human-readable label (not the raw id) after
// selection. This replaces Base-UI's default behavior of showing the raw value.
const LabelRegistryContext = React.createContext<{
...
function Select<Value>({ children, ...props }: SelectPrimitive.Root.Props<Value>) {
  ...
  const registryRef = React.useRef(new Map<unknown, React.ReactNode>())
  ...
  const api = React.useMemo(
    () => ({
      ...
      get: (value: unknown) => registryRef.current.get(value),
    }),
    []
  )

  return (
    <LabelRegistryContext.Provider value={api}>
  ...
  return (
    ...
    >
      {typeof children === "function"
        ? children
        : (value) => {
            const resolved = registry?.get(value)
